...
public class SOM {
	...
	private float[][][] nodes;//includes weights([][][0] and [][][1]) and category number([][][2])
	...
	public void iterationManager() {
		for(int t=0; t<iter; ++t) {
			...
			if (t==0 || t==20 || t==100 || t==1000 || t==5000 || t==iter-1){
	...
	public void updateNodes(int ic, int jc) {
		int roundRadius = (int) (radius + 1);//raius rounded up a whole node to not miss any possible in-radius nodes in efficient for loop. 
		int i2s = ic - roundRadius;
		int i2e = ic + roundRadius;
		int i1s = jc - roundRadius;
		int i1e = jc + roundRadius;
		
		//Modifies start and end points if the indices are out of the som boundary.
		if (i2s < 0) i2s = 0;
		if (i2e > n2som-1) i2e = n2som;
		if (i1s < 0) i1s = 0;
		if (i1e > n1som-1) i1e = n1som;
	...
	public void updateNode(int ic, int jc, int ii, int jj) {
		...
		if (distance==0){
			for (int i=0; i<n_att; ++i){
				...
				h = (float) (alpha*exp(-pow(distance(ic,jc,ii,jj),2)/(2.*pow(sigma,2))));
		...
		for (int i=0; i<n1; ++i){
			for (int j=0; j<n2; ++j){
				...
				if (eucDis < minEucDis){
					minEucDis = eucDis;
					winCatNum = node(j, i);
					//System.out.println("winCatNum["+i+"]["+j+"] = " +winCatNum);
