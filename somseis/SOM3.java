...
public class SOM3 {
	...
	public void iterationManager() {
		for(int t=0; t<iter; ++t) {
			...
			if (t==0 || t==20 || t==100 || t==1000 || t==5000 || t==100000){
	...
	public void updateNode(int ic, int jc, int kc, int ii, int jj, int kk) {
		...
		if (distance==0){
			for (int i=0; i<n_att; ++i){
				...
				h = (float) (alpha*exp(-pow(distanceTable[ic][jc][kc][ii][jj][kk],2)/(2.*pow(sigma,2))));

				//System.out.println("h= "+h);
				nodes[ic][jc][kc][i] = nodes[ic][jc][kc][i] + h*(pickedData[i] - nodes[ic][jc][kc][i]);
			}
		}
			...
			for (int i=0; i<n_att; ++i){
				...
				nodes[ii][jj][kk][i] = nodes[ii][jj][kk][i] + h*(pickedData[i] - nodes[ii][jj][kk][i]);
					...
					for (int i4=0; i4<n3som ; ++i4) 
						for (int i5=0; i5<n2som ; ++i5)
							for (int i6=0; i6<n1som ; ++i6){
								distanceTable[i1][i2][i3][i4][i5][i6] = (float) (sqrt(pow((i1-i4),2) + pow((i2-i5),2)) + pow((i3-i6),2));
							}
	}
	...
	private void findWinner(float[] singleTrainData){
		...
		for (int i=0; i<n3; ++i){
			for (int j=0; j<n2; ++j){
				for (int k=0; k<n1; ++k){
					float eucDis = euclideanDistance(singleTrainData, nodes[i][j][k]);
					if (eucDis < xmin){
						xmin = eucDis;
						ic = i;
						jc = j;
