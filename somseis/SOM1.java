...
public class SOM1 {
	...
	public void updateNode(int ic, int ii) {
		...
		if (distance==0){
			for (int i=0; i<n_att; ++i){
				...
				h = (float) (alpha*exp(-pow(distanceTable[ic][ii],2)/(2.*pow(sigma,2))));

				//System.out.println("h= "+h);
				nodes[ic][i] = nodes[ic][i] + h*(pickedData[i] - nodes[ic][i]);
			...
			for (int i=0; i<n_att; ++i){
				...
				nodes[ii][i] = nodes[ii][i] + h*(pickedData[i] - nodes[ii][i]);
	...
	public ColorMap getColorMap(int min, int max){
		...
		if (n1som < 256){
			...
			si.interpolate(n1som, lengthRatio, 0.0, hsbOrig[0], 256, 1.0, 0.0, hsbInterp[0]);
			si.interpolate(n1som, lengthRatio, 0.0, hsbOrig[1], 256, 1.0, 0.0, hsbInterp[1]);
		}
